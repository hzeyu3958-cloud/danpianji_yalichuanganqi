...
当 ADC 原始值低于 25，程序认为传感器没有有效接触，脉搏曲线归零并清除 BPM；有效接触后，带通信号包络还必须达到 6 ADC 以上，才允许波峰参与心率计算。这样没有放手指或只有电噪声时不会误报心率。不同传感器若静态 ADC 明显低于或高于此范围，可在源码中的 `CONTACT_ADC_MIN` 调整门限。
